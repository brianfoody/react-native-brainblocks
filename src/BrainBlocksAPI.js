 ...
 */
BrainBlocksAPI.startPaymentAsync = async (
  amountInRai: number,
  destination: string
): Promise<SessionSuccessType> => {
  const details = {
    amount: amountInRai,
 ...
 * @param {string} token
 */
BrainBlocksAPI.verifyPayment = async (
  token: string
): Promise<VerifyPaymentSuccessType> => {
  const options = {
    method: 'get',
  };
 ...
 * @param {string} amount
 */
BrainBlocksAPI.convertToRai = async (
  amount: number,
  currency: string
): Promise<ExchangeSuccessType> => {
  const options = {
    method: 'get',
  };
  ...
  let response = await fetch(
    `https://brainblocks.io/api/exchange/${currency}/${amount}/rai`,
    options
  );
  let responseJson = await response.json();
 ...
 * @param {string} token
 */
BrainBlocksAPI.waitOnTransfer = async (
  token: string
): Promise<TransferSuccessType> => {
  const options = {
    method: 'post',
  };

  let response = await fetch(
    `https://brainblocks.io/api/session/${token}/transfer`,
    options
  );
  let responseJson = await response.json();
